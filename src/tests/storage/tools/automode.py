...
class AutoMode(Manager, AutoBenchmarks):
    def __init__(self, serial, spreadsheet_id, sheet_identifier, client_file_name, logger=None):
        Manager.__init__(self, serial, logger=logger or getLogger(__file__))
        AutoBenchmarks.__init__(self, spreadsheet_id, sheet_identifier, client_file_name)
        
    def updateHeaderConfig(self):
        """
        Update default values of "self.header_config" to actual values
        """
        ...
        def addHeaderValue(index, value):
            if len(self.header_config[index]) > 3: 
                ...
                self.header_config[index][3].append(value)
