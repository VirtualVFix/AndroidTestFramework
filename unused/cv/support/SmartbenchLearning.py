...
#                    image = np.copy(frame)
                    ...
                    # find ditigs area for crop image
                    threshold = cv2.inRange(cv2.cvtColor(np.copy(frame), cv2.COLOR_RGB2HSV), cv.CV_RGB(0,20,110), cv.CV_RGB(140, 150, 135))
                    ...
                    cont = np.vstack(contours[i] for i in xrange(len(contours)))
...
#                    frame = cv2.resize(frame, (int(frame.shape[WIDTH]/IMAGE_SCALE), int(frame.shape[HEIGHT]/IMAGE_SCALE)))
                ...
                #===============================================================       
                drawimage = np.copy(frame)
                image = np.zeros((200,500))
#                image2 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
#                image = cv2.GaussianBlur(np.copy(image),(5,5),0)
                # crop image
#                threshold = cv2.inRange(cv2.cvtColor(image, cv2.COLOR_RGB2HSV), cv.CV_RGB(0,20,110), cv.CV_RGB(170, 150, 135))
#                threshold = cv2.GaussianBlur(threshold,(5,5),0)

                # binary crop image

#                threshold = cv2.threshold(image2, 90, 256, cv2.THRESH_TOZERO)[1]
                gray = cv2.cvtColor(np.copy(drawimage), cv2.COLOR_BGR2GRAY)   
#                hist = cv2.equalizeHist(gray)
#                threshold = np.hstack((gray,hist))
#                gray = cv2.GaussianBlur(np.copy(gray),(3,3),1)
#                threshold = cv2.threshold(gray, 120, 256, cv2.THRESH_TOZERO)[1]
                threshold = cv2.threshold(gray, 140, 256, cv2.THRESH_BINARY)[1]

#                threshold = cv2.inRange(cv2.cvtColor(np.copy(drawimage), cv2.COLOR_RGB2HSV), cv.CV_RGB(0,20,110), cv.CV_RGB(140, 150, 135))
#                threshold = cv2.Canny(np.copy(gray),150,200)
                ...
                for component in zip(contours, hierarchy):
                    ...
                    if currentHierarchy[2] > 0 or currentHierarchy[3] < 0: 
                        ...
                        cv2.rectangle(drawimage, (x,y), (x+w,y+h), (0,255,0), 1) # parent contour
                    else:
                        cv2.rectangle(drawimage, (x,y), (x+w,y+h), (0,0,255), 1) # child contour
...
#                                          thickness = 1, color = (0,255,255))
                ...
                self.drawText(image, color = (255,0,255))
                cv2.imshow(IMAGE_NAME, drawimage)
                cv2.imshow(IMAGE_NAME+'2', image)
