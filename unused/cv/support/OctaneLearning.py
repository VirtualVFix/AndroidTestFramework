...
#                image = cv2.GaussianBlur(np.copy(image),(5,5),0)
                # crop image
#                image2 = cv2.inRange(cv2.cvtColor(image, cv2.COLOR_RGB2HSV), cv.CV_RGB(210,160,90), cv.CV_RGB(256, 195, 130))

                # binary crop image
                threshold = cv2.threshold(image2, 155, 255, cv2.THRESH_BINARY_INV)[1]
#                threshold = cv2.adaptiveThreshold(image,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,11,7)
...
#                    if cv2.contourArea(np.int0(cv2.cv.BoxPoints(cv2.minAreaRect(currentContour)))) > 100:
                    if currentHierarchy[2] > 0 or currentHierarchy[3] < 0: 
                        _sample_area.append((x,y,w,h))
#                            print cv2.contourArea(np.int0(cv2.cv.BoxPoints(cv2.minAreaRect(currentContour))))
...
#                sample_area = sorted(sample_area, key=lambda x:x[0])
                ...
                for rect in _sample_area:
                    if rect[1]+rect[3] >= _max-20: sample_area.append(rect)
                sample_area = sorted(sample_area, key=lambda x:x[0])
                
                for x,y,w,h in sample_area:
                    cv2.rectangle(image, (x,y), (x+w,y+h), (255,255,0), 1) # parent contour
#                        
#                # get sample of symbol
                if current_selection < len(sample_area): 
                    sample = self.getSample(threshold, sample_area, current_selection) 
                    
#                samples = np.concatenate([x for x in sample], axis=1)
